...
public class DescriptionPanel extends JPanel {
    ...
    public void paintComponent(Graphics graphics) {
        ...
        TextLayout largeMachineName =
                new TextLayout(
                        this.machine.getName(),
                        new Font("Helvetica", Font.PLAIN, 100),
                        canvas.getFontRenderContext());
        canvas.setClip(0, 0, (int) clipBounds.getWidth(), 75);
        AffineTransform transform = new AffineTransform();
        transform.setToTranslation(
                (float) this.getWidth() - (float) largeMachineName.getBounds().getWidth(), 90.0D);
        ...
        transform.setToTranslation(
                (float) this.getWidth()
                        - (float)
                                new TextLayout(
                                                this.machine.getName(),
                                                new Font("Helvetica", Font.PLAIN, 50),
                                                canvas.getFontRenderContext())
                                        .getBounds()
                                        .getWidth()
                        - 50.0F,
        ...
        LineBreakMeasurer measurer =
                new LineBreakMeasurer(this.paragraph, canvas.getFontRenderContext());
    ...
    private void applyParagraph(
            Graphics2D canvas, LineBreakMeasurer measurer, float wrappingWidth) {
        TextLayout nextLine;
        float yOffset = 100.0f;
        for (int endIndex = this.paragraph.getEndIndex();
                measurer.getPosition() < endIndex;
                yOffset += nextLine.getDescent() + nextLine.getLeading()) {
            nextLine = measurer.nextLayout(wrappingWidth);
            yOffset += nextLine.getAscent() * 2.0F;
            nextLine.draw(
                    canvas,
                    nextLine.isLeftToRight()
                            ? (float) 200.0
                            : wrappingWidth - nextLine.getAdvance(),
                    yOffset);
        }
    }
    ...
    private void applyPaint(Graphics2D canvas) {
        canvas.setPaint(
                new GradientPaint(0.0F, 0.0F, new Color(204, 218, 234), 165.0F, 0.0F, Color.white));
    }
