...
public class Hand {
  ...
  public Hand(URL imageUrl, Point2D pointerOrigin, Point2D handLocation, ImageObserver imageObserver) {
  ...
  void moveTo(int keyframes, double x, double y) {
    Double startLocation = new Double(this.location.getX() + this.hotSpot.getX(), this.location.getY() + this.hotSpot.getY());
  ...
  void pause() {
    moveTo(10, this.location.getX() + this.hotSpot.getX(), this.location.getY() + this.hotSpot.getY());
  }

  void squiggle(int noKeyframes, Rectangle cell) {
    Double step1 = new Double(this.location.getX() + this.hotSpot.getX(), this.location.getY() + this.hotSpot.getY() - cell.getHeight());
    ...
    Double step5 = new Double(this.location.getX() + this.hotSpot.getX(), this.location.getY() + this.hotSpot.getY());
    Point2D[] path = new Point2D[]{step1, step2, step3, step4, step5};
    int[] frames = new int[]{0, noKeyframes / 4, noKeyframes / 4 * 2, noKeyframes / 4 * 3, noKeyframes};
    this.animation = new KeyframeAnimation(path, frames);
  }
