...
public class Hand {
  ...
  public Hand(
      URL imageUrl, Point2D pointerOrigin, Point2D handLocation, ImageObserver imageObserver) {
  ...
  public Point2D getLocation() {
    return new Double(
        this.location.getX() + this.hotSpot.getX(), this.location.getY() + this.hotSpot.getY());
  }
  ...
  void pause() {
    moveTo(
        10, this.location.getX() + this.hotSpot.getX(), this.location.getY() + this.hotSpot.getY());
  }
  ...
  void squiggle(int noKeyframes, Rectangle cell) {
    ...
    int[] frames =
        new int[] {0, noKeyframes / 4, noKeyframes / 4 * 2, noKeyframes / 4 * 3, noKeyframes};
    this.animation = new KeyframeAnimation(path, frames);
  }
