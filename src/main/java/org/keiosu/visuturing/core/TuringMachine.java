...
public class TuringMachine implements XmlElement {
    ...
    public TuringMachine() {
        ...
        this.alphabet =
                List.of(String.valueOf(Symbols.LEFT_END_MARKER), String.valueOf(Symbols.SPACE));
    }
        ...
        for (State state : this.states) {
            if (!state.getName().equals(Symbols.STATE_HALTING_STATE)) {
                for (String s : tempAlphabet) {
                    ...
                    List<Configuration> possibleNextConfigurations =
                            this.getNextConfig(configuration);
                ...
                if (nextConfiguration.getIndex() < nextConfiguration.getWord().length() - 1) {
                    word =
                            word
                                    + nextConfiguration
                                            .getWord()
                                            .substring(nextConfiguration.getIndex() + 1);
                }
    ...
    public String toXml() {
        ...
        for (String symbol : this.alphabet) {
            xml.append("<symbol>")
                    .append(Symbols.toUnicode(symbol.charAt(0)))
                    .append("</symbol>\n");
        }
        xml.append("</alphabet>\n");
            ...
            if (!transition.getCurrentState().equals(transition.getNextState())) {
                curveCenterPoint =
                        new Point(
                                ...
                                (int)
                                        ((double) maxHeight
                                                + (xToState - xFromState) / curveRatio / 2.0D));
            } else if (curveRatio % 2.0D == 0.0D) {
                curveCenterPoint =
                        new Point2D.Double(xFromState, (double) maxHeight - curveRatio * 60.0D);
            } else {
                curveCenterPoint =
                        new Point2D.Double(xFromState, (double) maxHeight + curveRatio * 60.0D);
            }
